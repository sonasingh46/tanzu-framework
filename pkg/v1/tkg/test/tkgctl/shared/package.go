...
// getClients gets the various kubernetes clients and gvrhelper
func getClients(ctx context.Context, exportFile string) (k8sClient client.Client, dynamicClient dynamic.Interface, aggregatedAPIResourcesClient client.Client, gvrHelper addonutil.GVRHelper, err error) {
	...
	gvrHelper = addonutil.NewGVRHelper(ctx, clientset.DiscoveryClient)
...
func checkClusterCB(ctx context.Context, mccl, wccl client.Client, mcClusterName, mcClusterNamespace, wcClusterName, wcClusterNamespace, infrastructureName string, isManagementCluster bool) error {
	...
	for _, pkg := range packages {
		if strings.Contains(pkg.Name, "kapp-controller") {
			verifyPackageInstall(ctx, mccl, pkg.Namespace, addonutil.GeneratePackageInstallName(clusterName, pkg.Name), pkg.Spec.RefName, pkg.Spec.Version)
		} else {
			verifyPackageInstall(ctx, client, pkg.Namespace, addonutil.GeneratePackageInstallName(clusterName, pkg.Name), pkg.Spec.RefName, pkg.Spec.Version)
		}
	}
...
func verifyClusterBootstrap(ctx context.Context, c client.Client, clusterBootstrap *runtanzuv1alpha3.ClusterBootstrap, tkrName string) {
	...
	for _, pkg := range packages {
		...
		if pkg.ValuesFrom != nil {
			if pkg.ValuesFrom.SecretRef != "" {
				pkg.ValuesFrom.SecretRef = addonutil.GeneratePackageSecretName(clusterBootstrap.Name, pkgShortName)
			} else if pkg.ValuesFrom.ProviderRef != nil {
				pkg.ValuesFrom.ProviderRef.Name = addonutil.GeneratePackageSecretName(clusterBootstrap.Name, pkgShortName)
			}
		}
	}
	...
	if strings.Contains(clusterBootstrap.Spec.CNI.RefName, "calico") {
		...
		expectedClusterBootstrap.Spec.CNI.ValuesFrom.ProviderRef = &corev1.TypedLocalObjectReference{
			APIGroup: &apiGroup,
			Kind: "CalicoConfig",
			Name: addonutil.GeneratePackageSecretName(clusterBootstrap.Name, "calico"),
		}
	}
 ...
 */
func getManagementClusterResources(ctx context.Context, mccl client.Client, dynamicClient dynamic.Interface, aggregatedAPIResourcesClient client.Client, gvrHelper addonutil.GVRHelper, clusterNamespace, clusterName, infrastructureName string) ([]clusterResource, error) {
	...
	for _, pkg := range packages {
		if pkg.ValuesFrom != nil {
			if pkg.ValuesFrom.Inline != nil {
				packageRefName, _, err := addonutil.GetPackageMetadata(ctx, aggregatedAPIResourcesClient, pkg.RefName, clusterNamespace)
				if err != nil {
					return nil, err
				}
				packageSecretName := addonutil.GeneratePackageSecretName(clusterName, packageRefName)
			...
			if pkg.ValuesFrom.ProviderRef != nil {
				gvr, err := gvrHelper.GetGVR(schema.GroupKind{Group: *pkg.ValuesFrom.ProviderRef.APIGroup, Kind: pkg.ValuesFrom.ProviderRef.Kind})
				if err != nil {
					return nil, err
				}
			...
			if infrastructureName == "vsphere" {
				ok, err := addonutil.IsTKGSCluster(ctx, dynamicClient, gvrHelper.GetDiscoveryClient(), &clusterapiv1beta1.Cluster{ObjectMeta: metav1.ObjectMeta{Name: clusterName, Namespace: clusterNamespace}})
				if err != nil {
					return nil, err
				}
				if ok {

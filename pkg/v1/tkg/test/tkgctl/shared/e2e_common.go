...
func E2ECommonSpec(ctx context.Context, inputGetter func() E2ECommonSpecInput) { //nolint:funlen
	...
	It("Should verify basic cluster lifecycle operations", func() {
		...
		if input.Plan == "devcc" || input.Plan == "prodcc" {
			...
			mngclient, mngDynamicClient, mngAggregatedAPIResourcesClient, mngGvrHelper, err := getClients(ctx, mngtempFilePath)
			...
			err = checkClusterCB(ctx, mngclient, wlcClient, input.E2EConfig.ManagementClusterName, constants.TkgNamespace, "","", input.E2EConfig.InfrastructureName, true)
			Expect(err).To(BeNil())
			...
			err = checkClusterCB(ctx, mngclient, wlcClient, input.E2EConfig.ManagementClusterName, constants.TkgNamespace, clusterName, namespace, input.E2EConfig.InfrastructureName, false)
			Expect(err).To(BeNil())
			...
			clusterResources, err = getManagementClusterResources(ctx, mngclient, mngDynamicClient, mngAggregatedAPIResourcesClient, mngGvrHelper, namespace, clusterName, input.E2EConfig.InfrastructureName)
			Expect(err).NotTo(HaveOccurred())
		}
